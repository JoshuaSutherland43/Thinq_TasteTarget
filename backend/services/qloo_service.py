...
async def call_qloo_api(product_info: dict) -> List[dict]:
    ...
    try:
        async with httpx.AsyncClient(timeout=15.0) as client:
            tag_mappings = {
                "sustainability": [
                    "urn:tag:genre:lifestyle:eco-friendly",
                    "urn:tag:genre:lifestyle:sustainable",
                ],
                "innovation": [
                    "urn:tag:genre:tech:innovative",
                    "urn:tag:genre:lifestyle:modern",
                ],
                "luxury": [
                    "urn:tag:genre:lifestyle:luxury",
                    "urn:tag:genre:lifestyle:premium",
                ],
                "minimalism": [
                    "urn:tag:genre:lifestyle:minimalist",
                    "urn:tag:genre:lifestyle:simple",
                ],
                "ethical": [
                    "urn:tag:genre:lifestyle:ethical",
                    "urn:tag:genre:lifestyle:conscious",
                ],
                "quality": [
                    "urn:tag:genre:lifestyle:quality",
                    "urn:tag:genre:lifestyle:premium",
                ],
            }
                ...
                try:
                    ...
                    params = {
                        ...
                        "signal.interests.tags": (
                            tags[0] if tags else f"urn:tag:genre:lifestyle:{value}"
                        ),
                    }
                    ...
                    if response.status_code == 200:
                        ...
                        logger.warning(
                            f"Qloo API returned {response.status_code} for {value}"
                        )
...
def convert_qloo_insights_to_cluster(
    insights: dict, cluster_name: str, index: int
) -> dict:
        ...
        if "data" in insights:
            ...
            for category in cluster["interests"]:
                if not cluster["interests"][category]:
                    cluster["interests"][category] = get_default_interests(
...
def get_default_interests(category: str, cluster_name: str) -> List[str]:
    ...
    return defaults.get(category, {}).get(cluster_name, defaults[category]["default"])[
        :5
    ]
...
async def mock_qloo_api(product_info: dict) -> List[dict]:
    ...
    if "sustainability" in product_info.get(
        "brand_values", []
    ) or "ethical" in product_info.get("brand_values", []):
        clusters.append(
            {
    ...
    if "innovation" in product_info.get(
        "brand_values", []
    ) or "modern" in product_info.get("brand_values", []):
        clusters.append(
            {
